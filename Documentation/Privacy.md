...
- Fixed failure context, stage and recovery categories when an operation fails or needs recovery.
- Fixed technical state categories used to diagnose relevant pairing and location failures.
- Background-session status and fixed permission or service-failure categories. No location values are included.
- iOS version in events sent to the self-hosted service.
- Limited technical configuration categories used to diagnose specific background-processing failures, sent only to the self-hosted service.
...
Release builds may be configured to send optional anonymous statistics to the destinations described above. Statistics are sent only when sharing is enabled in Roam Control.
